...
from server.utilities.constants import LOCAL_OUTPUT_CSS_DIR
from server.utilities.constants import LOCAL_OUTPUT_DIR
from server.utilities.constants import LOCAL_OUTPUT_GSAP_DIR
from server.utilities.constants import LOCAL_OUTPUT_JS_DIR
from server.utilities.constants import MINIO_ACCESS_KEY
from server.utilities.constants import MINIO_ARTICLE_BUCKET
from server.utilities.constants import MINIO_ENDPOINT
from server.utilities.constants import MINIO_SECRET_KEY
from server.utilities.constants import MINIO_SECURE
from server.utilities.constants import REGISTER_PLUGIN_TEMPLATE_PATH
...
async def upload_file(file: UploadFile, article_id: str) -> JSONResponse:
    try:
        path = stage_file(minio_client, article_id, file.file, file.filename, file.size, file.content_type)
...
async def generate_website(request_body: Article) -> JSONResponse:
    try:
        ...
        return JSONResponse(
            content={
                "message": f"Website generated successfully. The article can be found in \
                {'localhost:9000' if MINIO_ENDPOINT == 'minio:9000' else MINIO_ENDPOINT} \
                    /{MINIO_ARTICLE_BUCKET}/{request_body.article_id}/index.html"
            },
            status_code=status.HTTP_201_CREATED,
        )
...
async def root(request_body: Article) -> typing.Dict[str, str]:
    ...
    with tempfile.NamedTemporaryFile(delete=False, suffix=".html") as html_file, tempfile.NamedTemporaryFile(
        delete=False, suffix=".css"
    ) as css_file, tempfile.NamedTemporaryFile(delete=False, suffix=".js") as js_file:
    ...
    for plugin in plugins:
        generate_js_function(REGISTER_PLUGIN_TEMPLATE_PATH, plugin.local_register_path, plugin_name=plugin.plugin_name)
