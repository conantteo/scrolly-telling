...
from server.model.script.animation.animation_script_processor import AnimationScriptFactory, AnimationScriptProcessor
...
class ScriptProcessor:
    ...
    def process_pages(self, pages: list[Page]):
        for page in pages:
            frames = page.frames
    ...
    def __append_page_pinning_script(self, page: Page, component_groups: ComponentGroups):
        pin_js = PinScriptBuilder.build_pin_js(
            page.id,
            component_groups.get_biggest_group_size()
        )
        self.string_builder.append(pin_js)

    def __append_animation_scripts(self, component_groups: ComponentGroups):
        AnimationScriptProcessor.process_animation_scripts(
            component_groups,
            self.string_builder
        )

    def __process_frames(self, frames: list[Frame], component_groups: ComponentGroups):
        for frame in frames:
